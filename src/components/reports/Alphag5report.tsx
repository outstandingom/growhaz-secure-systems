...
const SCAN_LABELS: Record<string, { short: string; full: string }> = {
  "alpha-g1": { short: "Alpha G1", full: "Alpha G1 Essential" },
  "alpha-g2": { short: "Alpha G5", full: "Alpha G5 Professional" },
  "alpha-g5": { short: "Alpha G5", full: "Alpha G5 Professional" },
  "alpha-g5-pro": { short: "Alpha G5 Pro", full: "Alpha G5 Pro" },
  "alpha-g5-enterprise": { short: "Alpha G5 Enterprise", full: "Alpha G5 Enterprise" },
};

const SecurityReportComponent: React.FC<SecurityReportProps> = ({ report, scanType, onExport, onShare }) => {
  const scanLabel = SCAN_LABELS[scanType || ""] || SCAN_LABELS["alpha-g5"];
    ...
    try {
      ...
      const addHeaderToCurrentPage = () => {
        if (logoBase64) {
          ...
          doc.text(`${scanLabel.short} Security Report`, margin + logoWidth + 5, margin + logoHeight - 2);
        } else {
          // Fallback if no logo
          doc.setFontSize(16);
          doc.text(`${scanLabel.short} Security Report`, margin, margin);
        }
      ...
      const targetRows = [
        ...
        { label: 'Scan Type', value: scanLabel.full },
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(
          `Report generated by GROWHAZ ${scanLabel.full} Scanner on ${formatDate(report.timestamp)}`,
          margin,
          pageHeight - 10
        );
      ...
      <div className="bg-card rounded-xl border border-border p-4 sm:p-6 space-y-6 print:bg-white print:text-black print:border-0 print:shadow-none print:p-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 no-print">
          <h2 className="text-xl font-bold flex items-center gap-2">
            ...
            {scanLabel.short} Security Report
          </h2>
        ...
        <div className="hidden print:block mb-8">
          <div className="flex items-center justify-between border-b border-gray-300 pb-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">{scanLabel.short} Security Report</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          ...
          <div className="space-y-1">
            ...
            <p className="font-medium">{scanLabel.full}</p>
          </div>
          <div className="space-y-1">
        ...
        <div className="hidden print:block text-xs text-center text-gray-500 mt-8 pt-4 border-t border-gray-300">
          <p>Report generated by GROWHAZ {scanLabel.full} Scanner on {formatDate(report.timestamp)}</p>
