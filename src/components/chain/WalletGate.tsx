...
import { useChain, SEPOLIA_CHAIN_HEX, SEPOLIA_NAME } from "@/hooks/useChain";
...
export function WalletGate({ children }: { children: React.ReactNode }) {
  ...
  if (chainId && chainId.toLowerCase() !== SEPOLIA_CHAIN_HEX.toLowerCase()) {
    return (
      <div className="flex flex-col items-center gap-3 p-6 rounded-xl bg-amber-500/10 border border-amber-500/30">
        <AlertTriangle className="w-8 h-8 text-amber-400" />
        <p className="text-sm">Switch your wallet to <strong>{SEPOLIA_NAME}</strong>.</p>
        <Button size="sm" onClick={() => ensureSepolia()}>Switch to {SEPOLIA_NAME}</Button>
      </div>
    );
  }
  return <>{children}</>;
