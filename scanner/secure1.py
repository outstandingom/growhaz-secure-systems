#!/usr/bin/env python3
"""
Enhanced Security Testing Tool v2.3 (Final)
...
- Fixed emoji consistency and login 404 handling
...
class SecurityTester:
    ...
    def is_blocked(self, response):
        ...
        block_keywords = [
            ...
            'security challenge', 'robot', 'blocked'
        ]
    ...
    # ----------------------------------------------------------------------
    # Login attempt (now with cleaner 404 handling)
    # ----------------------------------------------------------------------
    def attempt_login(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log("Login endpoint not found (404). Skipping authenticated tests.", "INFO")
                return False
            else:
                self.log(f"Login failed: {response.status_code} - {response.reason}", "WARNING")
                return False
        except Exception as e:
    ...
    def test_authentication_flaws(self):
        ...
        for login_url in login_endpoints:
            for weak_pass in ["123456", "password", "admin", "qwerty", "test123"]:
        ...
        for test_name, test_func in tests:
            try:
                ...
                if result is True:
                    ...
                    self.log(f"{test_name}: 🚧 BLOCKED")
                elif result is False:
                    self.log(f"{test_name}: ✅ SECURE")
                else:
                    self.log(f"{test_name}: ⚠️ ERROR")
            except Exception as e:
