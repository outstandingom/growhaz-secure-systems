#!/usr/bin/env python3
"""
Enhanced Security Testing Tool v2.4 (Optimized & Threaded)
- JavaScript crawling (Playwright) for React/Next.js sites
- WAF circuit breaker – stops testing endpoints after repeated blocks
...
class SecurityTester:
    ...
    def save_report(self):
        ...
        report = {
            ...
            "test_summary": self.test_summary,          # New section with detailed test statuses
            "summary": {
    ...
    def measure_baseline(self, endpoint, method='GET', params=None, data=None, json_data=None, samples=3):
        ...
        for _ in range(samples):
            try:
                start = time.time()
                if method.upper() == 'GET':
                    self.session.get(endpoint, params=params, timeout=3)      # reduced timeout
                elif method.upper() == 'POST':
                    if json_data:
                        self.session.post(endpoint, json=json_data, timeout=3)
                    else:
                        self.session.post(endpoint, data=data, timeout=3)
    ...
    def attempt_login(self):
        ...
        for login_url in all_login_urls:
            try:
                response = self.session.post(login_url, json=data, timeout=3)   # reduced timeout
    ...
    def test_sql_injection(self):
        """Test for SQL injection vulnerabilities – optimized with circuit breaker and actual parameters"""
        self.log("🔍 Testing SQL Injection...")
        vulnerable = False
        test_details = []   # to build summary
        ...
        for endpoint in test_endpoints:
            ...
            for true_payload, false_payload in boolean_payloads:
                ...
                try:
                    ...
                    resp_true = self.session.post(endpoint, json=true_data, timeout=5)   # keep 5s for SQLi
                    if self.is_blocked(resp_true):
                        consecutive_blocks += 1
                        continue

                    false_data = {"email": false_payload, "password": "anything"}
                    resp_false = self.session.post(endpoint, json=false_data, timeout=5)
            ...
            # Time-based tests (continue even if boolean found, but we can break if we already marked vulnerable)
            # But to keep summary, we might still run other tests? For speed, we can continue.
    ...
    def test_xss(self):
        ...
        for endpoint in test_endpoints:
            ...
            params_to_test = actual_params if actual_params else ['q', 'id', 'search']   # minimal fallback
        ...
        def check_xss(endpoint, param, payload):
            ...
            try:
                ...
                if any(x in endpoint.lower() for x in ['search', 'q', 'query']):
                    resp = self.session.get(endpoint, params=data, timeout=3)
                else:
                    resp = self.session.post(endpoint, json=data, timeout=3)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_task):
                ...
                if result["type"] == "vulnerable":
                    ...
                    # We can optionally log that we skipped an endpoint due to blocks
    ...
    # ----------------------------------------------------------------------
    # IDOR test (with WAF check) – unchanged except circuit breaker and actual parameters
    # ----------------------------------------------------------------------
    def test_insecure_direct_object_reference(self):
        ...
        for endpoint_template in idor_endpoints:
            ...
            for test_id in test_ids:
                ...
                try:
                    resp = self.session.get(endpoint, timeout=3)
                    if self.is_blocked(resp):
                        consecutive_blocks += 1
                        continue
                    if resp.status_code == 200:
    ...
    # ----------------------------------------------------------------------
    # CORS test (with WAF check) – unchanged
    # ----------------------------------------------------------------------
    def test_cors_misconfiguration(self):
        ...
        try:
            ...
            for origin in test_origins:
                ...
                resp = self.session.options(self.base_url, headers=headers, timeout=3)
                if self.is_blocked(resp):
                    continue
                cors_headers = {
    ...
    # ----------------------------------------------------------------------
    # Sensitive Data Exposure (with WAF check) – unchanged except timeouts
    # ----------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------
    # Security Headers test (with WAF check) – unchanged
    # ----------------------------------------------------------------------
    ...
    def test_csrf(self):
        ...
        for endpoint in state_change_endpoints:
            ...
            try:
                ...
                valid_resp = self.session.post(endpoint, json=valid_data, timeout=3)
                if self.is_blocked(valid_resp):
                    consecutive_blocks += 1
                    continue
                if valid_resp.status_code == 200:
                    temp_session = requests.Session()
                    invalid_resp = temp_session.post(endpoint, json=valid_data, timeout=3)
            ...
            for payload in payloads:
                ...
                try:
                    if '?' in endpoint:
                        ...
                        url = endpoint + payload
                    resp = self.session.get(url, allow_redirects=False, timeout=3)
                    if self.is_blocked(resp):
                        consecutive_blocks += 1
                        continue
    ...
    # ----------------------------------------------------------------------
    # Directory Traversal test – unchanged
    # ----------------------------------------------------------------------
    ...
    def run_all_tests(self):
        ...
        # Print summary from self.test_summary
...
def main():
    ...
    print(r"""
    ╔══════════════════════════════════════════════════════════════╗
    ║     GROWHAZ Enhanced Security Testing Tool v2.4              ║
    ║     Optimized & Threaded | Supabase Integration              ║
    ╚══════════════════════════════════════════════════════════════╝
    """)
