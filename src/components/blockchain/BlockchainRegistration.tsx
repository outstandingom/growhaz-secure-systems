...
import { 
  ...
  Copy, ExternalLink, User, Shield, Clock, FileText, Save, X, RefreshCw
} from 'lucide-react';
import { USER_REGISTRY_V2_ADDRESS, USER_REGISTRY_V2_ABI, isUserRegistryV2Deployed } from '@/lib/userRegistryV2Contract';
import { BrowserProvider, Contract } from 'ethers';

// Sepolia explorer (adjust if you use another network)
...
interface BlockchainRegistrationProps {
  ...
  onWalletConnect?: (address: string, type: string) => void;
  onWalletDisconnect?: () => void;
  onRegistrationComplete?: (txHash: string) => void;
}
...
export function BlockchainRegistration({
  ...
  const [onChainProfile, setOnChainProfile] = useState<{ exists: boolean; ipfsCid: string; updatedAt: number } | null>(null);
  ...
  const fetchOnChainProfile = async () => {
    ...
    try {
      // Use read-only provider (no signer needed for view function)
      const provider = new BrowserProvider(window.ethereum);
      const contract = new Contract(USER_REGISTRY_V2_ADDRESS, USER_REGISTRY_V2_ABI, provider);
      const [ipfsHash, updatedAt] = await contract.getProfile(walletAddress);
      ...
      if (ipfsHash && ipfsHash !== '' && updatedAt > 0) {
        ...
        if (response.ok) {
          ...
          setIpfsProfileData(data);
          // Pre-fill form with existing data
          if (data.name) setBcName(data.name);
          if (data.phone) setBcPhone(data.phone);
          if (data.profession) setBcProfession(data.profession);
          if (data.work) setBcWork(data.work);
          if (data.age) setBcAge(data.age);
        }
      } else {
        setOnChainProfile({ exists: false, ipfsCid: '', updatedAt: 0 });
      }
    } catch (error) {
  ...
  const connectMetaMask = async () => {
    ...
    try {
      ...
      onWalletConnect?.(address, 'metamask');
  ...
  const handleRegisterOrUpdate = async () => {
    ...
    setRegistering(true);
    try {
      // 1. Upload profile to IPFS via Pinata
      ...
      const { error: dbError } = await supabase.from('blockchain_user_registrations').insert({
        ...
        event_type: onChainProfile?.exists ? 'ProfileUpdated' : 'UserRegistered',
      ...
      toast({
        title: onChainProfile?.exists ? '✅ Profile Updated!' : '✅ Blockchain Profile Created!',
        description: `TX: ${txHash.slice(0, 14)}... | IPFS: ${ipfsCid.slice(0, 12)}...`
      });

      onRegistrationComplete?.(txHash);
    } catch (error: any) {
  ...
  // Wallet connected, loading on-chain profile
  if (loadingOnChain) {
    return (
  ...
  // Wallet connected, no profile yet -> registration form
  if (walletAddress && !onChainProfile?.exists) {
    return (
      <div className="mb-6 p-4 rounded-xl border border-amber-500/30 bg-amber-500/5">
        ...
        <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-5 mb-3">
          <div className="space-y-1">
            ...
            <Input value={bcName} onChange={(e) => setBcName(e.target.value)} placeholder={userProfile?.full_name} className="bg-background/50 text-sm h-8" />
          </div>
          <div className="space-y-1">
            <Label className="text-[10px] text-muted-foreground">Phone</Label>
            <Input value={bcPhone} onChange={(e) => setBcPhone(e.target.value)} placeholder={userProfile?.phone || 'Optional'} className="bg-background/50 text-sm h-8" />
          </div>
          <div className="space-y-1">
            <Label className="text-[10px] text-muted-foreground">Profession *</Label>
            <Input value={bcProfession} onChange={(e) => setBcProfession(e.target.value)} placeholder="e.g. Developer" className="bg-background/50 text-sm h-8" />
          </div>
          <div className="space-y-1">
            <Label className="text-[10px] text-muted-foreground">Work / About</Label>
            <Input value={bcWork} onChange={(e) => setBcWork(e.target.value)} placeholder="e.g. Full-stack dev" className="bg-background/50 text-sm h-8" />
          </div>
          <div className="space-y-1">
            <Label className="text-[10px] text-muted-foreground">Age (18+) *</Label>
            <Input type="number" min="18" max="120" value={bcAge} onChange={(e) => setBcAge(e.target.value ? Number(e.target.value) : '')} placeholder="e.g. 28" className="bg-background/50 text-sm h-8" />
          </div>
        </div>
  ...
  // Wallet connected, profile exists -> display and update
  return (
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={fetchOnChainProfile} className="h-7 px-2">
              <RefreshCw className="w-3 h-3" />
            </Button>
            <Button variant="ghost" size="sm" onClick={handleDisconnect} className="h-7 px-2 text-destructive">
              <X className="w-3 h-3" />
            </Button>
            <a href={`https://gateway.pinata.cloud/ipfs/${onChainProfile.ipfsCid}`} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center gap-1">
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-5 mb-4">
          <div className="p-3 rounded-xl bg-card/60 border border-border">
            ...
            <p className="font-medium text-sm">{ipfsProfileData?.name || bcName || userProfile?.full_name}</p>
          </div>
          <div className="p-3 rounded-xl bg-card/60 border border-border">
            ...
            <p className="font-medium text-sm">{ipfsProfileData?.profession || bcProfession || 'Not set'}</p>
          </div>
          <div className="p-3 rounded-xl bg-card/60 border border-border">
            <span className="text-[10px] uppercase tracking-wider text-emerald-400 font-semibold flex items-center gap-1 mb-1">Age</span>
            <p className="font-medium text-sm">{ipfsProfileData?.age || bcAge || 'N/A'}</p>
          </div>
          <div className="p-3 rounded-xl bg-card/60 border border-border">
            ...
            <p className="font-medium text-sm">{new Date(onChainProfile.updatedAt * 1000).toLocaleDateString()}</p>
          </div>
          <div className="p-3 rounded-xl bg-card/60 border border-border">
        ...
        <div className="p-3 rounded-xl bg-card/60 border border-border mb-4">
          ...
          <div className="flex items-center gap-2">
            <code className="font-mono text-xs break-all flex-1">{onChainProfile.ipfsCid}</code>
            <button onClick={() => copyToClipboard(onChainProfile.ipfsCid)} className="text-muted-foreground hover:text-primary shrink-0"><Copy className="w-3 h-3" /></button>
          </div>
        </div>
        ...
        <div className="border-t border-border/30 pt-4">
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-5 mb-3">
            <div className="space-y-1">
              ...
              <Input value={bcName} onChange={(e) => setBcName(e.target.value)} placeholder={ipfsProfileData?.name} className="bg-background/50 text-sm h-8" />
            </div>
            <div className="space-y-1">
              <Label className="text-xs text-muted-foreground">Phone</Label>
              <Input value={bcPhone} onChange={(e) => setBcPhone(e.target.value)} placeholder={ipfsProfileData?.phone} className="bg-background/50 text-sm h-8" />
            </div>
            <div className="space-y-1">
              <Label className="text-xs text-muted-foreground">Profession</Label>
              <Input value={bcProfession} onChange={(e) => setBcProfession(e.target.value)} placeholder={ipfsProfileData?.profession} className="bg-background/50 text-sm h-8" />
            </div>
            <div className="space-y-1">
              <Label className="text-xs text-muted-foreground">Work/About</Label>
              <Input value={bcWork} onChange={(e) => setBcWork(e.target.value)} placeholder={ipfsProfileData?.work} className="bg-background/50 text-sm h-8" />
            </div>
            <div className="space-y-1">
              <Label className="text-xs text-muted-foreground">Age</Label>
              <Input type="number" min="18" max="120" value={bcAge} onChange={(e) => setBcAge(e.target.value ? Number(e.target.value) : '')} placeholder={ipfsProfileData?.age?.toString()} className="bg-background/50 text-sm h-8" />
            </div>
          </div>
